 ...
 */
package jchan16.ics4u.storageArea.Lite.tpj7;
...
//runs through  results array  to get indivual years with no repeats
    ...
    //runs through results array to get individual subjects
    public void getAllSubjects(){
    ArrayList<String> Subjects= new ArrayList<String>();
    //result array copies array2         
    sortSubject();
    sortSubjectGhost();
    //subject of the first element
    String  cursor =getArray2().get(0).getSubject();
        Subjects.add(cursor);
        for (int j = 1; j < getArray2().size(); j++) {
            cursor = getArray2().get(j).getSubject();
            //getArray2().get(i) is compared to all years inside year array
            //if it's different counter doesn't change  if not counter subtracts by 1
            int counter = 0;
            for (int i = 0; i < getYearArray().size(); i++) {
                if (cursor.equalsIgnoreCase(Subjects.get(i))==true) {
                    counter--;
                }
                if (cursor.equalsIgnoreCase(Subjects.get(i))==true) {
                    counter++;
...
//works just like searchGainsGhost instead if the entry has a true to t checker variable, it removes the element from results array
    public void searchLossesGhost() {
        ...
        int j = 0;
        while (j < g) {
            if (getResults().get(j).showEORL() == true) {
                getResults().remove(j);
                g = getResults().size();
            } else {
                i++;
...
//use method only if results array is filled, goes through all of array, if element entry subject doesn not match inputted, it is removed
    public void searchSubjectGhost(String a1) {
        ...
        int j = 0;
        while (j < g) {
            if (a1.equalsIgnoreCase(getResults().get(j).getSubject()) == false) {
                getResults().remove(j);
                g = getResults().size();
            } else {
                i++;
...
//works just like searchDate(int,int) but runs through results array instead of search array
    public void searchDateGhost(int a, int b) {
        ...
        if (a < b) {
            ...
            int j = 0;
            while (j < g) {
                if (getResults().get(j).date2(4) < a || getResults().get(j).date2(4) > b) {
                    getResults().remove(j);
                    g = getResults().size();
                } else {
                    i++;
...
//works just like searchID but only goes through results array than search array
    public void searchIDGhost(int a, int b) {
        ...
        if (a < b) {
            ...
            while (j < g) {
                if (getResults().get(j).getId() < a || getResults().get(j).getId() > b) {
                    getResults().remove(j);
                    g = getResults().size();
                } else {
                    i++;
...
//Works just like searchFavorite only runs through results array rather than seacrh array
    public void searchFavoriteGhost(boolean b) {
        ...
        if (b == true) {
            ...
            //searches through all of the array
            while (j < g) {
                if (getResults().get(j).getFavorite() == false) {
                    getResults().remove(j);
                    g = getResults().size();
//                    System.out.println("\n");
//                    printResults();
            ...
            int j = 0;
            while (j < g) {
                if (getResults().get(j).getFavorite() == true) {
                    getResults().remove(j);
                    g = getResults().size();
                } else {
                    i++;
...
//works similarily to searchAmount(double,double) but only runs through results array and removes elements that don't fit paramter 
    public void searchAmountGhost(double a, double b) {
        if (a < b) {
            ...
            //searches through the whole array
            while (j < g) {
                if (getResults().get(j).showPrice() < a || getResults().get(j).showPrice() > b) {
                    getResults().remove(j);
                    g = getResults().size();
                } else {
                    i++;
    ...
    public void searchAmountGhost(String a, double b) {
        ...
        switch (a) {
            case "<":
                ...
                //searches through the whole array
                while (j < g) {
                    if (getResults().get(j).showPrice() > b) {
                        getResults().remove(j);
                        g = getResults().size();
                    } else {
                        i++;
...
//subbstracts total gains by total losses
    ...
    //adds all amounts in entries already in  results array
    public double getTotalGhost(){
        double total=0;
        for(int i=0; i<getResults().size();i++){
            //adds to total if entry gained money
            if(getResults().get(i).showEORL()==true){
                total+= (double)getResults().get(i).showPrice();
                //subtracts money if entry is a loss
            }else if(getResults().get(i).showEORL()==false){
                total-= (double)getResults().get(i).showPrice();
            }    
        }
        return total;
