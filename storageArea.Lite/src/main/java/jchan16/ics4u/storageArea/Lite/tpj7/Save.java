...
//USed in tandom with get File Names, the int just returns the position in listFiles of wanted file.Not that useful right now
    ...
    public int getLogPlace(int a) {
     ...
     public int getLogPlace2(String a) {
        for (int i = 1; i < parent().listFiles().length; i++) {
            String b = parent().listFiles()[i].getName().substring(0, (parent().listFiles()[i].getName().length()-4));
...
//    return "Does not exists";
    ...
    public String getFileName2(String a) {
        try {
            return parent().listFiles()[getLogPlace2(a)].getAbsolutePath();
        } catch (ArrayIndexOutOfBoundsException e) {
            return "Does Not Exist";
        }
//    return "Does not exists";
...
//            System.out.println("reee");
    ...
    public void getEntryTotal2(File g) {
        int count = 0;
        int total = 0;
        if (g.listFiles().length == 1) {
//            System.out.println("reee");
            ...
            try {
                BufferedReader v = new BufferedReader(new FileReader(parent().listFiles()[g.listFiles().length - 1]));
                String f = v.readLine();
                while (f != null) {
                    if (f.equalsIgnoreCase("null") == false) {
                        count++;
